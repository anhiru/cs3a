"""
sample rgb program using
3-tuples of floats which hold the rgb values for a color
2-tuples containing a string (name of color) and the 3-tuples (its rgb values)
a larger list to collect the 2-tuples of various colors
"""
...
def get_rgb_value():
    ...
    r, g, b = "", "", ""
    while not accepted:
        user_color = input("Enter red, green and blue values of your color,"
                           " separated by a space.\n* Legal values run from"
                           " 0.0 (min) to 1.0 (max)*\n> ")
        try:
...
def color_dist(rgb_1, rgb_2):
    ...
    if (type(rgb_1) != tuple or type(rgb_2) != tuple
            or len(rgb_1) != 3 or len(rgb_2) != 3):
        return None

    dist = 0
    for i in range(3):
        dist += (rgb_2[i] - rgb_1[i]) ** 2

    return dist


def find_closest_match(rgb_val, color_list):
    """find the color in RGB_COLORS that is the smallest distance away"""
...
print(("\nThe pre-packaged color that most closely matches your custom design"
       " is '{}',\nwhose RGB values are red = {}, green = {}, blue = {}\n").
